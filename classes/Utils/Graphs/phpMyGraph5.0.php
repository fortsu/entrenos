...
//-------------------------------------------------------------------------+
	 ...
	 * @return void
	 */
	public static function parseVerticalPolygonGraph($data, $cfg = array(), $altitude_offset = 0) {
		$graph = self::factory('verticalPolygonGraph', $data, $cfg);
		$graph->parse($data, $cfg, $altitude_offset);
	}
...
//-------------------------------------------------------------------------+
	...
	*/	
	protected function parseDataStructure($data, $altitude_offset = 0) {
		//Test
		if(!is_array($data)) {
        ...
        //error_log("max: " . $structure['max'] . " | min: " . $structure['min'],0);        

        // dgranda - added on 20110718
        if ($altitude_offset > 0) {
            $graph_step_tmp = floor(($structure['max']-$structure['min'])/6);
            if ($graph_step_tmp > 5) {
                $graph_step = floor($graph_step_tmp/5)*5;
            } else {
                $graph_step = 5;
            }
            $structure['fakeMax'] = ceil($structure['max']/$graph_step)*$graph_step;
        } else { 
...
/*
        ...
        if ($altitude_offset > 0) {
            foreach($data as $key => $value) {
                $new_data[$key] = $value - $altitude_offset;
            }
        }
        $structure['data'] = $new_data;
*/
        $structure['fakeMax'] += $altitude_offset;
        $structure['fakeMin'] += $altitude_offset;
		//Result
		return $structure;
	}
...
//-------------------------------------------------------------------------+
	...
	*/	
	protected function drawZero($graphPoints) {
		//Test visible
		if($this->cfg['zero-line-visible'] AND FALSE) {
...
//-------------------------------------------------------------------------+
	 ...
	 * @comment: Parses the graph
	*/	
	public function parse($data, $cfg = array(), $altitude_offset = 0){
		try {
			//Parse config
			$this->parseConfig($cfg);

			//Get data structure
			$dataStructure = $this->parseDataStructure($data, $altitude_offset);
