...
function gmgetdate($ts = null){
    ...
    return(array_comb($k,explode(":",gmdate('s:i:G:j:w:n:Y:z:l:F:U',is_null($ts)?time():$ts)))); // split deprecated in php 5.3
	} 
...
class Spreadsheet_Excel_Reader {
	...
	function _format_value($format,$num,$f) {
		...
		//$parts = split(";",$format); // split method is deprecated in php 5.3
			...
			switch ($code) {
				case SPREADSHEET_EXCEL_READER_TYPE_SST:
					...
					for ($i = 0; $i < $uniqueStrings; $i++) {
                        ...
                        $retstr = ($asciiEncoding) ? iconv('cp1250',$this->_defaultEncoding,$retstr) : $this->_encodeUTF16($retstr);
...
//						$bgcolor = ($bgi & 0x3f80) >> 7;
    ...
    /*
        ...
        Credits to http://www.devirtuoso.com/2010/08/reading-excel-documents-with-php-and-ajax/
    */
    function dump2array($sheet=0) {
        $array_sheet = array();
        ...
        for($row=1;$row<=$this->rowcount($sheet);$row++) {
            ...
            for($col=1;$col<=$this->colcount($sheet);$col++){
                ...
                $array_sheet[$row][$col] = htmlentities($this->val($row,$col,$sheet));
            }
        }
        return $array_sheet;
    }
